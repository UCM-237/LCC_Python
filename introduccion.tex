...
\index[eng]{Conversion! decimal to binary. Integer numbers} How to convert a number representations from decimal to binary? One more time, the key is to look at the number representation as a digits multiplied by powers of two. Let us start with an integer number. SUpose we have a number D, represented in decimal basis. We can expand it a sum of powers of two. If we divide the nuber by two, we could represent it as:
\end{paracol} 
\begin{equation*}
\label{eq:1}
D=2\cdot C_1+R_1
...
\index[eng]{Conversion! denary to binary, non-integer numbers} Suppose now, that we have non-integer number in denary representation as $0.d$. If we multiply the number by two:
\end{paracol}
...
El error cometido es, en este caso: $\text{Error}=0,623-0,62109375=0,00190625$.
 ...
 \sectionmark{Apls. al software cientifíco \textreferencemark \ Software Scientific Apps.}
