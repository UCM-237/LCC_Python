...
The function \mintinline{python}{firsttime} searches for the first time that a number \mintinline{python}{b} appears in a list \mintinline{python}{A}. The \mintinline{python}{for} loop starts at line $55$. The \mintinline{python}{if} structure, enclosed in the \mintinline{python}{for} loop, checks if the element \mintinline{python}{i} of the list meets the wanted number. if so, the program executes the \mintinline{python}{break} command, and the program returns the position \mintinline{python}{p} of the number in the list.  If the element \mintinline{python}{i} of the list \mintinline{python}{A} does not match the wanted number, the program goes back to the beginning of the \mintinline{python}{for} loop and will try the next element of the list. Notice that if the list \mintinline{python}{A} hasn't got the number, the position retrieved will be $-1$.
\end{paracol}
