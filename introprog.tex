...
\begin{paracol}{2}
La funcion \mintinline{python}{sumalist}, toma como variable de entrada una lísta de números de cualquier longitud y devuelve como salida la suma de todos los números contenidos en la lista. Además, imprime en el terminal de Ipython cada una de los números. El bucle \mintinline{python}{for} comienza en la línea $22$. La primera vez que el código llega a dicha línea asigna a la variable \mintinline{python}{i} el primer elemento de la lista \mintinline{python}{A}. En la linea $23$ muestra por pantalla el valor dicho elemento. En la línea $24$ suma su contendido a la variable \mintinline{python}{y} y vuelve de nuevo a la línea $22$, Asigna a la variable \mintinline{python}{y} el valor del segundo elemento de \mintinline{python}{A}. El proceso se repite hasta que se han recorrido todos los elementos de la lista. Es entonces cuando el programa termina el bucle y ejecuta la linea $25$, devolviendo el valor de la variable $y$, en la que se ha acumulado la suma de los elementos de la lista.

La función \mintinline{python}{sumaelementos}, toma coma variables de entrada dos listas de igual longitud y devuelve como variable de salida una lista cuyos elementos contienen las sumas de los elementos de las listas de entrada que ocupan la misma posición. El código está protegido por una estructura condicional, de modo que si las lista de entrada no tienen la misma longitud el programa muestra un mensaje por pantalla y devuelve una lista vacía. Si las listas tienen la misma longitud, en la línea $41$ se crea la variable \mintinline{python}{index} empleando el comando \mintinline{python}{range}. Por tanto, \mintinline{python}{index} sera una lista que contendrá los valores enteros desde $0$ hasta la $longitud -1$ de la s listas de entrada. En la línea $42$ se inicia el bucle \mintinline{python}{for} sobre dicha lista \mintinline{python}{i in index}. En la línea $42$ se suman los elementos de las listas de entrada que ocupan la posición indicada por el valos de \mintinline{python}{i} y se añade el resultado a la lista \mintinline{python}{S}. Cuando el bucle ha recorrido y sumado todos los elementos de las listas de entrada, el bucle termina y se devuelve el valor de \mintinline{python}{S}. Esta manera de crear una lista de índices para recorrer los elementos de una estructura de datos se usa con mucha frecuencia en programación.
...
\paragraph{List comprehension.} \index{List comprehesion}Python cuenta con excelentes recursos para realizar operaciones de modo iterativo sobre diversos tipos de estructuras de datos. Entre ellos, vamos a ver una conocida con el nombre de \emph{List comprehension}. La idea es crear una lista, en la que los elementos se obtienen como el resultado de una iteración en otra u otras listas. Su estructura general toma la forma:
\end{paracol}

\mint{python}{newlist = [expression for item in iterable if condition == True]}

\begin{paracol}{2}
La nueva lista se forma a partir de una operación (expresión) que se realiza sobre cada elemento (item) de otra estructura de datos (iterable). Además se puede imponer una condición sobre cada elemento para que se lleva a cabo la operación. El siguinete script de Python, contiene un par de ejemplos,
\end{paracol}
\inputminted[
frame=lines,
framesep=2mm,
...
\begin{paracol}{2}
La función \mintinline{python}{lc_demo}, Crea una lista con los números impares contendidos en otra lista. ES fácil la estructura de la \emph{list comprehesion}, creada en la línea $18$: la lista \mintinline{python}{B}, está formada por los elementos \mintinline{python}{i} incluidos en la lista \mintinline{python}{A}, que cumplan \mintinline{python}{i != 0}, es decir, que no sean divisibles por dos y, por tanto, impares. 

La segunda función, \mintinline{python}{pair_cmh}, es un poco más compleja, pero deja ver las posibilidades del método. La función crea una lista de duplas, para ello emplea dos estructuras \mintinline{python}{for}, tal y como están escritas, es como si fueran dos \mintinline{python}{for} anidados. Así, la funcion compara  cada elemento \mintinline{python}{i} de la primera lista con todos los elementos \mintinline{python}{j} de la segunda y, si son distintos \mintinline{python}{i != j}, guarda los dos números como una duplas \mintinline{python}{(i,j)} en la lista \mintinline{python}{pares.}

\paragraph{bucle while.} \index{Flujo! Bucle while}\index{Bucles! Bucle while} Este bucle tiene la misma finalidad que un bucle \texttt{for}, repetir un trozo de código un determinado número de veces. Lo que cambia, es el mecanismo que determina cuantas iteraciones realizará el bucle. En el caso de un bucle \texttt{while} las iteraciones se repiten un número indefinido de veces mientras se cumpla una determinada condición impuesta al principio del bucle. La figura \ref{fig:while} muestra la estructura general de un bucle while. Como sucede en todas las estructuras de programación que hemos visto en Python, es preciso que el código correspondiente al bucle \mintinline{python}{while} esté indentado. El siguiente código muestra algunos ejemplos de uso,
\end{paracol}
...
El siguiente código muestra dos funciones para calcular los $n$ primeros números primos La primera emplea una variable de aviso para detectar cuando un número impar no es divisible por todos los primos anteriores a él.
